...
export const useDepartamentoLogic = (selectedInstitucion: string) => {
  ...
  useEffect(() => {
    const fetchDepartamentos = async () => {
      ...
      try {
        // Log para depuración
        console.log(
          `Intentando obtener departamentos para institución: ${selectedInstitucion}`
        );

        // Construir URL completa para depuración
        const url = `${API_BASE_URL}/Combos/instituciones/${selectedInstitucion}/departamentos`;
        ...
        Swal.fire({
          icon: "error",
          title: "Error",
          text: `No se pudieron cargar los departamentos. ${error.message}`,
        });
      } finally {
        setIsLoading(false);
      }
